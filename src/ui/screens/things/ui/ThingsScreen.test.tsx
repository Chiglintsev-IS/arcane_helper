 ...
 * покупки показывают то, чего в сумке нет. Показано одно из трёх, и выбранное переживает перезапуск.
 */
...
describe("«Вещи» (FR-304)", () => {
  it("«Вещи» показывают одно из трёх, и переключатель называет какое (FR-304)", async () => {
  ...
  it("выбранное в «Вещах» переживает перезапуск (FR-304)", async () => {
    const user = userEvent.setup();
    await renderWithStores(<ThingsScreen />);
