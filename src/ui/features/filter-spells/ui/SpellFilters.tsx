 ...
 *
 * Время накладывания и роль делят в полосе одно место. В «Игре» стоит роль: в бою спрашивают,
...
export function SpellFilters({
  ...
  /**
   * Режим нужен ради подготовки и цены, которые спрашивают только в «Книге», и ради места, которое
   * делят время накладывания с ролью.
   */
  mode: ScreenMode;
  ...
  const castingTimes = inBook
    ? CASTING_TIME_FILTERS.filter((value) => dividing.castingTimes.has(value))
    : [];
