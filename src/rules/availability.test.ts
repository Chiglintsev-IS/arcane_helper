...
describe("checkAvailability: компоненты (FR-030, OQ-06)", () => {
  it("перечисляет вербальный и соматический компоненты напоминанием", () => {
    expect(check({ spell: mageArmor }).componentReminders).toEqual([
      "Вербальный: нужно говорить",
      "Соматический: нужна свободная рука",
      "Материальный: кусок обработанной кожи",
    ]);
  });
  ...
  it("предупреждает, что фокусировка не заменяет компонент со стоимостью", () => {
    ...
    expect(reminders).toContain(
      "Материальный: жемчужина стоимостью не менее 100 зм и перо совы — стоимость 100 зм, фокусировка не заменяет",
    );
  });
