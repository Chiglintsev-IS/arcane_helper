...
/**
 * Ссылочная целостность: подготовленное и книга ссылаются на существующие карточки (FR-121).
 *
...
export function checkIntegrity(
  ...
  for (const field of ["cantripIds", "spellbookSpellIds", "preparedSpellIds"] as const) {
    for (const id of character[field]) {
      if (!known.has(id)) {
        return `В наборе «${field}» есть заклинание «${id}», карточки для которого в файле нет.`;
      }
    }
  }
  return null;
