/**
 * Мастер кровавого колдовства (FR-177, FR-178).
 *
 * Обмен хитов на очки стоит строкой в том же списке, что и заклинания, тратит то же действие — и
 * потому проходит тот же мастер ([BloodMagicRow](../combat/BloodMagicRow.tsx),
 * [FR-207](../../../docs/features/F-18-screen-modes.md)). Раньше строка списывала до 18 хитов одним
 * тапом: две соседние строки одного списка вели себя по-разному, и та, что дороже ошибается, была
 * без подтверждения.
 *
 * Оболочка — общая с мастером заклинаний ([ADR-0017](../../../docs/decisions.md#adr-0017)):
 * одинаковость экрана держится одним компонентом, а не копированием классов.
 *
 * Инвариант FR-022 держится тем же способом, что и у `castDraftStore`, — отсутствием доступа:
 * выбранный объём живёт в локальном состоянии, состояние персонажа отсюда менять нечем. Меняет его
 * только `onConfirm`, и вызывается он ровно из кнопки подтверждения.
 */
 ...
 * ([rules-engine.md](../../../docs/rules-engine.md#кровавое-колдовство)).
 */
...
/** Подсказка «на что хватит»: очки живут до долгого отдыха, поэтому остаток тоже считается (FR-175). */
...
export function BloodMagicWizard({
  ...
  character: CharacterState;
  economy: TurnEconomy;
  /** Единственное действие мастера, меняющее состояние персонажа (FR-022). */
  onConfirm: (points: number, allowAnyway: boolean) => void;
  onCancel: () => void;
  error: string | null;
}) {
  const rate = ascensionTierRate(character.level);
  // Обмен до нуля разрешён — он даёт раны, но это решение игрока (FR-031, FR-175).
