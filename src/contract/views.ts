...
const itemViewSchema = z.object({
  ...
  /**
   * Те же прибавки фактами: равная прибавка всем величинам семейства названа одним фактом, а не
   * столькими, сколько в семействе величин. Что считать семейством, решают правила — показывающий
   * названное не пересобирает и обратно не разворачивает. Правят прибавки перечнем выше: там у
   * каждой величины своё число, и правка целится в величину, а не в семейство.
   */
  bonusFacts: z.array(
    z.object({
      /** Про что прибавка: про одну величину (`stat`) или про целое семейство (`family`). */
      kind: word,
