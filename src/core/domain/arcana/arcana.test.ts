...
describe("списание очков заклинаний", () => {
  it("пустой запас не списывается: владелец пула проверяет это сам", () => {
    // Оплата кровью добирает недостающее до списания, но пул отвечает за себя и без неё.
    expect(() => arcana().spendSpellPoints(1)).toThrow(/Очков заклинаний 0, нужно 2/);
  });

  it("разрешение мастера пускает пул в долг", () => {
    expect(arcana().spendSpellPoints(1, { allowAnyway: true }).toState().spellPoints.remaining).toBe(
      -2,
    );
  });
});
