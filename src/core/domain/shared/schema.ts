...
/**
 * Причина отказа объявления — по-русски: без неё `.int()`, `.min()` и прочие проверки без своего
 * текста говорят по умолчанию английским. Передаётся каждым разбором схемы владельца, а не разом
 * всем zod: смена языка библиотеки на глобальную не должна задевать чужой код, который её зовёт.
 */
export const russianSchemaErrors = z.locales.ru().localeError;
