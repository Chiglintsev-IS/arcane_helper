/**
 * Концентрация: КС проверки при получении урона.
 *
 * Сложность — половина полученного урона, но не меньше десяти.
 */
...
import { plural, SAVING_THROW_NAMES, withPlural } from "@/core/shared/language";
...
/**
 * Раунд, в котором начался эффект: столько ходов началось к его времени.
 *
 * Считается так же, как раунд в экономии хода — по записям о начале хода. Журнал обрезается
 *, поэтому у долгого эффекта начало может быть потеряно: тогда число помечается неточным.
 */
 ...
 * Приложение не бросает кубик — бросает игрок.
 */
...
/** Кто бросает и против чего. Числа готовые: игрок называет их вслух. */
...
/**
 * Готовое описание активной концентрации.
 *
