...
## Задачи 3–10 — рёбра к нулю (строго по порядку)
...
## Задача 14 — хвосты документации (параллельна задачам 3–9: код не пересекается)
...
3 → 4 → … → 10 → 11 → 13 → 15. Задача 12 независима (только UI-слой) — параллельно задачам
3–9 в worktree; задача 14 независима (только docs) — параллельно им же. Задачи 11 и 13 — строго
после 10: задача 10 переписывает пути импортов в тех же файлах use cases и UI. Внутри 3–10
порядок обязателен: задача 3 даёт shared-модули задачам 4–8, задача 10 собирает всё.
