...
- `5ff787c` — базлайн вернулся к досессионному содержимому: он оказался переписан по ходу работы без
  ...
  не влияло: он считается по коду, а в файл только заглядывает.
...
## Задачи 9 и 10 — рёбра к нулю (строго по порядку)
...
## Задача 14 — хвосты документации (параллельна задаче 9: код не пересекается)
...
9 → 10 → 11 → 13 → 15. Задача 12 независима (только UI-слой) — параллельно задаче
9 в worktree; задача 14 независима (только docs) — параллельно им же. Задачи 11 и 13 — строго
после 10: задача 10 переписывает пути импортов в тех же файлах use cases и UI. Задача 10 идёт последней: она собирает всё, что разложила задача 9.
