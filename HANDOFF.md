...
## Задачи 6–10 — рёбра к нулю (строго по порядку)
...
## Задача 14 — хвосты документации (параллельна задачам 6–9: код не пересекается)
...
6 → 7 → … → 10 → 11 → 13 → 15. Задача 12 независима (только UI-слой) — параллельно задачам
6–9 в worktree; задача 14 независима (только docs) — параллельно им же. Задачи 11 и 13 — строго
после 10: задача 10 переписывает пути импортов в тех же файлах use cases и UI. Внутри 6–10
порядок обязателен: задача 10 собирает всё, что разложат задачи 6–9.
