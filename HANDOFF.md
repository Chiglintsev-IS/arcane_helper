...
Прогон целиком зелёный: 1484 теста, typecheck, `check:docs --strict-link-remnants`,
`check:layers` (долг 15 рёбер / 9 циклов).
...
## Задача 1 — регресс «Привала» и чтение режима (первая: видна игроку)

Берёт код задачи 2 прежней редакции (оболочка и экраны); правит `src/ui/screens/rest/`,
`src/ui/app/PlayShell.tsx`, `src/core/shared/screenMode.ts`.

1. `src/ui/screens/rest/ui/RestScreen.tsx` — четыре пустых обработчика (`onOpenArmorClass`,
   `onOpenHitPoints`, `onEditResources`, `onOpenConcentration`, строки 55–57 и 63): в старом
   `PlayScreen` эти плитки на «Привале» открывали шторки, теперь нажатие не делает ничего.
   Завести шторки у самого экрана — правило «Экран владеет своими шторками» (`docs/screens.md`).
   Компонентные тесты на каждое нажатие.
2. `src/ui/app/PlayShell.tsx` — `readMode` кастует значение localStorage `as ScreenMode` без
   сверки со `SCREEN_MODES`: битое значение даёт режим, не попадающий ни в один `case`, и рендер
   падает. Валидировать по списку с откатом в `DEFAULT_SCREEN_MODE`, доступ к localStorage — в
   `try/catch` (приватный режим Safari бросает). Убрать мёртвую переменную `economy` (строка 69).
   Тест: битый ключ → приложение открывается в «Игре».
3. `src/core/shared/screenMode.ts` — докстринг утверждает, что значение сохраняется с сессией и
   мигрирует; ни того, ни другого больше нет. Поправить или перенести модуль в `ui/shared`
   (режим — состояние интерфейса по ADR-0034; выбрать по месту, глоссарий — тем же коммитом).
...
- `glossary.md`: `screenMode` как поля больше нет; `reactionAvailable` — теперь производное
  проекции, не хранимое; добавить `shortRestSinceLongRest`, `CURRENCY_ABBREVIATIONS`,
  `Equipment.idFromName`.
...
- `ux.md:68` «приложение живёт одним экраном» → одна страница без маршрутов, оболочка и шесть
  ...
  «Игру»; `screens.md:438` (FR-204) — режим живёт в оболочке, не в сохранении, и назвать
  фактический e2e. `architecture.md:64` — режим не «сохраняемое значение»; `:69` — назвать
  оболочку в описании `ui/app`.
...
1 → 2 → 3 → … → 10 → 11 → 13 → 15. Задача 12 независима (только UI-слой) — параллельно задачам
