...
- `CampActions.tsx` — `exchangeWarnings` рядом с обменом ещё читает курс ступени ради фразы «N хитов
  за очко»: это число, а не правило, но уезжает вместе с самой функцией (первая строка задачи).
