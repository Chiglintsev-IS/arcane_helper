...
- `npm run test:e2e` в этот список не входил, и именно там пряталась вёрстка: прогонять его каждой
  задаче, которая трогает `src/ui/**`. Красными остаются два прогона бюджета — задача 1а.
...
Прогон зелёный: 1495 тестов, typecheck, `check:docs --strict-link-remnants`, `check:layers`
(долг 15 рёбер / 9 циклов), сборка. E2E — 17 из 19; два красных описаны задачей 1а.

## Задача 1а — бюджет вёрстки на iPhone SE (видна игроку, требует решения по внешнему виду)

Правит `src/ui/app/PlayShell.tsx`, `src/ui/screens/game/`, шапку ресурсов и полосу фильтров.
Красные прогоны: E2E `combat keeps the first card whole, the book keeps the first row` и
`concentration block explains the effect`. Оба падали ещё до задачи 1 — их не видели потому, что
e2e не входил в обязательный прогон.

Измерено на 320 × 568 после «Начать бой», без концентрации: шапка оболочки 73 (переключатель 48 +
`p-3` сверху и снизу + рамка), шапка ресурсов 183, полоса фильтров 92, список начинается на 477, и
первая карточка высотой 136 кончается на 612 — на 44 ниже края. С активной концентрацией
непрокручиваемая часть сама выше экрана: документ 584 против 568, и прокручивается вся страница.

До разделения на оболочку и экраны переключатель, шапка ресурсов, блок действующего и ряд кнопок
стояли одним блоком с одним `p-3`; теперь у оболочки свой `p-3` с рамкой, а верхний блок «Игры»
потерял и `px-3` — шапка ушла в край экрана. Тринадцать пикселей возвращаются сюда, остальные
тридцать — решение игрока: что урезать (высоту переключателя, полосу фильтров, высоту карточки в
бою). Решение — в `ADR`, число — в требование; `FR-218` до этого стоит «В работе».
...
1а → 2 → 3 → … → 10 → 11 → 13 → 15. Задача 12 независима (только UI-слой) — параллельно задачам
