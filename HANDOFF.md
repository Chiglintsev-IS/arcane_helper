...
## Задачи 7–10 — рёбра к нулю (строго по порядку)
...
## Задача 14 — хвосты документации (параллельна задачам 7–9: код не пересекается)
...
7 → 8 → 9 → 10 → 11 → 13 → 15. Задача 12 независима (только UI-слой) — параллельно задачам
7–9 в worktree; задача 14 независима (только docs) — параллельно им же. Задачи 11 и 13 — строго
после 10: задача 10 переписывает пути импортов в тех же файлах use cases и UI. Внутри 7–10
порядок обязателен: задача 10 собирает всё, что разложат задачи 7–9.
