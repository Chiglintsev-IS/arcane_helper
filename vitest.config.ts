...
export default defineConfig({
  ...
  test: {
    ...
    environment: "node",
    /*
     * Предел времени прогона стоит против зависшего, а не против медленного. Прогон экрана
     * поднимает оболочку целиком и нажимает по одной клавише за раз: на ноутбуке самый долгий из
     * них идёт около трёх секунд, а на машине сборки — втрое дольше, и пятисекундный предел по
     * умолчанию рубил честные прогоны через раз. Публикация от этого падала случайным образом, а
     * новая версия не доезжала до телефона.
     */
    testTimeout: 20_000,
