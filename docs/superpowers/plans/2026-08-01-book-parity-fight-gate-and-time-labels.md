...
### Task 8: FR-217 — шапка «Книги» очищается до того, чем в ней пользуются

> Добавлено 2026-08-01 по замечанию игрока со снимком экрана, после задач 1-5 и 7.

**Files:**
- Modify: `src/components/combat/ResourceHeader.tsx` (строка имени и уровня)
- Modify: `src/components/combat/CombatScreen.tsx` (кнопки «Поиск» и «Реакции»)
- Modify: `src/components/combat/SpellFilters.tsx` (уровни в одну прокручиваемую строку)
- Modify: `docs/features/F-18-screen-modes.md` (FR-217, FR-212), `docs/open-questions.md` (новый OQ)
- Test: `src/components/combat/CombatScreen.test.tsx`, `src/components/combat/SpellFilters.test.tsx`, `e2e/uc-01-cast-spell.spec.ts`

**Замечание игрока:** «на экране книги есть лишние элементы, которые не нужны самой книге, чтобы читать её, подготавливать заклинания и применять их вне боя. Нужно убрать имя "Торн" и уровень, "Поиск", "Реакции", "Отменить" и "Очки". Уровень ячеек может быть под одной строкой со скроллом, остальные фильтры как есть».

**Что из этого делается, а что нет.** Два элемента из шести остаются, и оба по решению игрока после того, как ему была названа цена:

| Элемент | Решение |
|---|---|
| Имя «Торн», класс и уровень | убрать в «Книге» |
| Кнопка «Поиск» | убрать в «Книге» |
| Кнопка «Реакции» | убрать в «Книге», оставить в «Бою» и «Вне боя» |
| Переключатели уровня | одна прокручиваемая строка, остальные фильтры переносятся как раньше |
| Значок «Очки» | **остаётся** — очки в книге покупают, и результат покупки должен быть виден (FR-207) |
| Кнопка «Отменить» | **остаётся до появления вкладки «Журнал»** — это единственный вход к обратимости (FR-111) |

- [ ] **Step 1: Написать падающие компонентные тесты**

В `src/components/combat/CombatScreen.test.tsx` добавить блок `describe("шапка «Книги» без лишнего (FR-217)")` с проверками, что в режиме «Книга»:

- нет заголовка с именем персонажа: `expect(screen.queryByRole("heading", { name: "Торн" })).toBeNull();`
- нет текста класса и уровня: `expect(screen.queryByText(/Волшебник, 7 уровень/)).toBeNull();`
- нет кнопки «Поиск»: `expect(screen.queryByRole("button", { name: "Поиск" })).toBeNull();`
- нет кнопки «Реакции»: `expect(screen.queryByRole("button", { name: "Реакции" })).toBeNull();`
- значок очков и кнопка отмены на месте: `expect(within(screen.getByRole("region", { name: "Ресурсы" })).getByText(/Очки 0/)).toBeDefined();` и `expect(screen.getByRole("button", { name: /Отменить/ })).toBeDefined();`
- счётчик подготовки на месте: он и был ответом на вопрос «сколько ещё можно» (FR-214).

И проверки, что в «Бою» и «Вне боя» имя, класс, уровень и кнопка «Реакции» остались. Существующий тест задачи 4 `«Знаки ограждения» вне боя (FR-153)` ожидает кнопку во всех трёх режимах — его ожидание для «Книги» поправить на её отсутствие, а два остальных режима оставить.

В `src/components/combat/SpellFilters.test.tsx` — проверка, что переключатели уровня лежат в отдельном контейнере с горизонтальной прокруткой, а остальные переключатели по-прежнему в переносящейся полосе. Проверять структуру, а не CSS-классы: дать контейнеру уровней `aria-label="Уровень"` и утверждать про него.

- [ ] **Step 2: Убедиться, что тесты падают**

Run: `npx vitest run src/components/combat/CombatScreen.test.tsx src/components/combat/SpellFilters.test.tsx`
Expected: FAIL.

- [ ] **Step 3: Убрать имя, класс и уровень в «Книге»**

В `ResourceHeader.tsx` обернуть `<header>` с `<h1>{character.name}</h1>` и строкой класса в проверку режима. Комментарий назвать причину: в «Книге» читают и готовятся, а имя персонажа на вопрос «чем сегодня платить» не отвечает — и это тот же довод, которым из «Книги» убраны КС, атака, КД и хиты (FR-217).

Счётчик раундов в этой строке уже выводится из `turnTracked` и в «Книге» отсутствует сам — отдельной ветки не нужно.

- [ ] **Step 4: Убрать «Поиск» и «Реакции» в «Книге»**

В `CombatScreen.tsx`:
- кнопку «Поиск» и поле ввода убрать в «Книге», то есть убрать вовсе: `searchOpen`/`query` показывались только при `preparing`. Убрать и сами состояния, и всё, что от них осталось мёртвым, включая ветку `matchesQuery` в `bloodShown` и блок `findBan`, **если** после удаления они становятся недостижимы. Проверить это, а не предположить: `query` участвует ещё и в сообщении о пустом результате.
- кнопку «Реакции» показывать при `character.screenMode !== "book"`.

**Осторожно: удаление поиска уносит единственный вход к [FR-162](../../features/F-14-campaign-restrictions.md#fr-162)** — объяснению, почему запрещённого заклинания нет в списке («Понимание языков запрещено мастером»). Требование не отменяется и код его не теряет молча: завести запись в `docs/open-questions.md` (следующий свободный номер `OQ-34`) с формулировкой «Где живёт объяснение запрета кампании после того, как поиск ушёл из «Книги»» и ссылкой на FR-162 и на эту задачу. Ветку `findBan` не удалять, пока вопрос открыт, — удалить только вход, если он стал недостижим, и назвать это в записи.

- [ ] **Step 5: Уровни в одну прокручиваемую строку**

В `SpellFilters.tsx` вынести переключатели уровня в отдельный контейнер с `aria-label="Уровень"`, `overflow-x-auto`, `flex-nowrap` и `shrink-0` на каждом переключателе. Остальные переключатели оставить в переносящейся полосе.

Дописать причину в комментарий и не стирать прежнюю: FR-212 требует, чтобы полоса переносилась, а не прокручивалась, потому что уехавший за край переключатель в бою — переключатель, которого нет. Для уровней в «Книге» довод другой, и он должен быть записан: уровней пять, они идут подряд одним рядом чисел, читаются как шкала, а не как набор, и в «Книге» их оглядывают не под чужой ход. Правило боя при этом не меняется — фильтров уровня в бою нет вовсе (FR-212).

- [ ] **Step 6: Прогнать тесты и E2E**

Run: `npm run test` затем `npm run test:e2e`
Expected: PASS. E2E-проверки, которые нажимали «Поиск» или искали имя персонажа в «Книге», поправить на новый состав; замер FR-218 должен стать только свободнее.

- [ ] **Step 7: Записать в спеку**

В FR-217 дописать абзац о том, что шапка «Книги» лишилась имени, класса и уровня, с доводом из шага 3 и с указанием, что значок очков остался по FR-207. В FR-212 дописать абзац про прокручиваемую строку уровней с доводом из шага 5. Порядок сверху вниз, перечисленный в FR-217, привести к фактическому.

Отдельно записать в FR-217, что кнопки «Поиск» и «Реакции» в «Книге» нет, а «Отменить» остаётся до появления отдельного места для журнала — со ссылкой на FR-111 и на то, что это решение игрока, а не недоделка.

- [ ] **Step 8: Проверить целостность спеки и закоммитить**

```bash
npm run check:docs
git add src/components/combat/ResourceHeader.tsx src/components/combat/CombatScreen.tsx \
        src/components/combat/SpellFilters.tsx src/components/combat/CombatScreen.test.tsx \
        src/components/combat/SpellFilters.test.tsx e2e/uc-01-cast-spell.spec.ts \
        docs/features/F-18-screen-modes.md docs/open-questions.md
git commit -m "Strip the book header down to what the book is for (FR-217)"
```

---

### Task 9: FR-131 — магическое восстановление требует короткого отдыха

> Добавлено 2026-08-01 по вопросу игрока: «как связано магическое восстановление? что нужно делать во время него?»

**Files:**
- Modify: `src/store/session.ts` (`useArcaneRecovery`, `shortRest`, `longRest`)
- Modify: `src/data/schemas/character.ts` (признак «отдыхал после последнего восстановления», если он нужен)
- Modify: `src/components/combat/CampActions.tsx` (подпись короткого отдыха и условие кнопки)
- Modify: `docs/features/F-06-resources.md` (FR-131, FR-132)
- Test: `src/store/session.test.ts`, `src/components/combat/CombatScreen.test.tsx`

**Что сломано.** [FR-131](../../features/F-06-resources.md#fr-131) говорит «один раз между долгими отдыхами, **после короткого отдыха**». Код проверяет только `arcaneRecoveryAvailable` (`session.ts:1149`) — отдыхал игрок или нет, кнопка работает. Код противоречит собственному требованию, а это отклонение, а не мелочь.

**Что ещё неточно.** Короткий отдых по PHB — отрезок **не менее часа**, а не ровно час: прервали раньше — отдых не считается. Подпись кнопки говорит «час», и игрок спросил прямо, «иногда это не час, а меньше». Подпись должна называть минимум.

- [ ] **Step 1: Написать падающие тесты**

В `src/store/session.test.ts`: магическое восстановление до короткого отдыха отказывает с причиной; после короткого отдыха проходит; после использования снова требует отдыха; долгий отдых возвращает и признак доступности, и необходимость отдыха.

В `CombatScreen.test.tsx`: в режиме «Вне боя» кнопки магического восстановления нет, пока не нажат короткий отдых; после короткого отдыха она появляется. Это прямое следствие FR-215 «показывается только то, что сейчас сделать можно».

- [ ] **Step 2: Убедиться, что тесты падают**

Run: `npx vitest run src/store/session.test.ts src/components/combat/CombatScreen.test.tsx`
Expected: FAIL.

- [ ] **Step 3: Связать восстановление с отдыхом**

Признак вывести из журнала, а не хранить: последняя запись о коротком или долгом отдыхе позже последней записи о магическом восстановлении — значит восстановление доступно. Хранимый флаг разошёлся бы с журналом ровно так же, как когда-то разошлись флаги экономии хода ([ADR-0008](../../decisions.md#adr-0008)), и отмена по FR-111 его бы не вернула.

Поэтому: `arcaneRecoveryAvailable` в состоянии **оставить** — он отвечает на «один раз между долгими отдыхами», — а новое условие «после короткого отдыха» считать из журнала отдельной функцией рядом с `deriveTurnEconomy`. Если из журнала это вывести нельзя, остановиться и доложить, а не заводить второй флаг.

Отказ называть причиной: «Магическое восстановление идёт после короткого отдыха».

- [ ] **Step 4: Поправить подпись короткого отдыха**

В `CampActions.tsx` подпись короткого отдыха назвать минимум: «не менее часа» вместо «час». Долгий отдых — «не менее 8 часов». Числа взяты из PHB 5e и в спеке уже стоят; меняется только слово.

- [ ] **Step 5: Прогнать тесты**

Run: `npm run test`
Expected: PASS. Сценарии, которые вызывали магическое восстановление без отдыха, привести к настоящему порядку: сначала короткий отдых.

- [ ] **Step 6: Записать в спеку**

В FR-131 добавить, что связь с коротким отдыхом теперь соблюдается кодом, и назвать, откуда берётся признак — из журнала, а не из флага, со ссылкой на ADR-0008. В FR-132 уточнить «не менее часа» и назвать, что отдых открывает магическое восстановление.

- [ ] **Step 7: Проверить целостность спеки и закоммитить**

```bash
npm run check:docs
git add src/store/session.ts src/store/session.test.ts src/components/combat/CampActions.tsx \
        src/components/combat/CombatScreen.test.tsx docs/features/F-06-resources.md
git commit -m "Require a short rest before arcane recovery (FR-131)"
```
