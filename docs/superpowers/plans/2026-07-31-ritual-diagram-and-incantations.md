...
```bash
python3 scripts/check-docs.py   # «спецификация целостна»
npm run typecheck               # чисто
npx vitest run                  # всё зелёное
```

Базовая линия чистая, поэтому критерий для каждой задачи ниже простой: **ноль ошибок целостности**. Если появилась ошибка — её принесла текущая задача.

Одно замечание про сам скрипт. Он обходит все `.md` репозитория, а каталог `docs/superpowers/` — черновики рабочего процесса: они цитируют требования, которых ещё нет, и содержат образцы строк спеки для вставки в другие файлы. От этого срабатывали все шесть правил сразу: и «FR упоминается, но нигде не определено», и «неизвестный статус», и битые ссылки — потому что относительные пути внутри плана резолвятся от каталога плана, а не от места, куда текст будет вставлен. Каталог исключён из обхода (`DRAFTS` в `scripts/check-docs.py`), источник истины — `docs/` без него.
