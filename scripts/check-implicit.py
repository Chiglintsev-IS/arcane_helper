...
Честный способ обойтись без приведения почти всегда есть, и он же лучше: объявить прежнюю форму
схемой, а не обещать её приведением; спросить ключи у того, кто их объявил; сузить тип объявлением
там, где приведение расширяло; читать непроверенные данные полями, значения которых так и остаются
`unknown`. Там, где способа нет, дыра языка закрывается у операции, которая ею владеет, и называется
по имени — словарь исключений ниже, и каждая запись несёт обоснование. Запись без находки — тоже
ошибка: исключение, которому нечего разрешать, обещает опасность там, где её уже нет.
"""
