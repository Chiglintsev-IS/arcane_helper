...
Что проверяется:
  ...
  3. Сокращения монет и мер, размер кости строкой, а также слова кнопок, которыми набранное
     вступает в силу или уходит ни с чем, стоят только у своего владельца: словарь языка знает «зм»,
     «см», «мм», размер кости приходит из состояния, а «Сохранить», «Подтвердить», «Отмена» и
     «Вернуть» — из словаря отображения.
  ...
  5. Отображение не проверяет доменных инвариантов: ни `Number.isInteger`, ни сравнения с пределом
     ...
     чтение, а не решение.
  6. Обводка называет роль заклинания и приходит от её владельца; ступень подложки — от владельца
     ступеней. Раздать обводку разделу, кнопке и полю значит не раздать её никому: единственная
     осмысленная обводка читается как ещё одна декоративная.
...
# Владелец обводки: роль заклинания. Обводка в приложении одна, и красит её он.
FRAME_OWNER = "src/ui/entities/spell/ui/SpellCardCompact.tsx"
# Линия края (`border-t`, `border-b`) не обводит: она отмечает край закреплённого над прокруткой.
...
def check_frame_owner(paths: list[pathlib.Path]) -> None:
    """Обводка называет роль заклинания, и владелец у неё один.

    Обводка притягивает взгляд сильнее любого другого приёма: раздать её разделу, кнопке, полю и
    метке значит не раздать её никому — единственная осмысленная обводка читается как ещё одна
    декоративная. Структуру держат ступень подложки и отступ, и они же приходят от владельца.

    Линия края не обводит и потому разрешена: она отмечает, что содержимое уходит под закреплённое.
    """
    for path in paths:
        ...
        for number, value in literals_of(path.read_text(encoding="utf-8"), ANY_LITERAL):
            for token in value.split():
                if FRAME_CLASS.fullmatch(token):
                    errors.append(f"{path}:{number}: обводка мимо владельца роли — «{token}»")


def check_surface_owner(paths: list[pathlib.Path]) -> None:
    """Ступень подложки приходит от владельца: набранная руками расходится с ним молча."""
    for path in paths:
        if not str(path).startswith("src/ui/") or str(path) == SURFACE_OWNER:
            continue
        for number, value in literals_of(path.read_text(encoding="utf-8"), ANY_LITERAL):
            for token in value.split():
                if SURFACE_CLASS.fullmatch(token):
...
def check_owned_literals(paths: list[pathlib.Path]) -> None:
    ...
    for path in paths:
        ...
        for number, value in literals_of(text, ANY_LITERAL):
            ...
            if str(path) != BUTTON_LABELS_OWNER and value.strip() in BUTTON_LABELS:
                errors.append(f"{path}:{number}: слово кнопки мимо словаря — «{value[:40]}»")
