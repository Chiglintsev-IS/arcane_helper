...
#
# Названия алхимических эффектов: перечень справочника называет их все, а начальное содержимое
# персонажа — те, которые стол уже раскрыл его отряду. Сверки между ними нет и быть не может:
# свойство приходит словами мастера, и он волен назвать своё, которого в перечне не окажется.
# Направление алхимии: перечень направлений — их владелец, содержимое персонажа называет то, которым
# стол пометил его свойство, а таблица тонов — цвет, которым это направление читается на экране.
# Сверка здесь есть: схема состояния принимает только направление из перечня.
ALLOWED_REPEATS: set[str] = {
    "Зельеварение / Особое",
    "Лечение здоровья",
