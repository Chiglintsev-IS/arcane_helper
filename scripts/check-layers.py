...
Что проверяется:
 ...
 19. Отображение видит из логики только договор: `src/ui/**` берёт из `src/` либо договор, либо себя.
     Исключения названы поимённо, и каждое несёт обоснование.
...
Девятнадцатое — то, ради чего заведён договор. Пока отображение видит логику, «вся логика в ядре»
держится дисциплиной: замыкание, переданное в сценарий, и агрегат, исполненный в компоненте,
выглядят как обычный вызов и не отличимы от чтения проекции ни одним признаком, кроме внимания
читающего. Исключения перечислены поимённо ниже — их три, и четвёртое означало бы, что правило
перестало проверять границу: исключение, которое нельзя назвать, и есть отсутствие правила.

Импорт считается и отложенный: `import("@/…")` приводит модуль в тот же бандл, только позже, и
правило, которое видит одну статическую форму, обходится добавленной скобкой.
...
# Импорт бывает статическим и отложенным. Отложенный — такое же пересечение границы: он приводит
# модуль в тот же бандл, только позже, и правило, которое его не видит, обходится одной скобкой.
IMPORT = re.compile(r'(?:from|import)\s*\(?\s*["\']@/([^"\']+)["\']')
TYPE_IMPORT = re.compile(r'import\s+type\s+[^;]*?["\']@/([^"\']+)["\']', re.S)
ANY_IMPORT = re.compile(r'(?:from|import)\s*\(?\s*["\'](@/[^"\']+|\.[^"\']*)["\']')
...
# Корень договора: единственное, что отображение видит из логики.
CONTRACT_LAYER = "contract"

# Файлы отображения, которым позволено видеть логику. Каждая запись — с обоснованием.
UI_ASSEMBLY = {
    # Композиционный корень: он и есть то место, где выбирают провод и собирают ядро. Видеть все
    # слои сразу — его работа, и других таких мест нет.
    "ui/app/providers/stores.tsx",
    # Прогонная сборка сторов: собирает настоящее ядро на хранилище в памяти. Подделка вместо него
    # проверяла бы подделку.
    "ui/app/testing/stores.tsx",
}

# Модули вне договора, которые отображению позволено брать. Каждая запись — с обоснованием.
UI_ALLOWED_MODULES = {
    # Язык: по-русски говорят обе стороны, и вторая копия склонения разошлась бы с первой молча.
    # Морфология не правило игры и не предметная область — потому и живёт отдельным корнем.
    "shared/language",
}
...
def check_imports(path: pathlib.Path) -> None:
    ...
    for target in IMPORT.findall(text):
        ...
        if (
            source_layer.startswith("ui/")
            and not target_layer.startswith("ui/")
            and target_layer != CONTRACT_LAYER
            and relative not in UI_ASSEMBLY
            and target not in UI_ALLOWED_MODULES
        ):
            errors.append(
                f"{path}: отображение видит из логики только договор — @/{target}; "
                f"нужное едет проекцией, а исключения названы в проверке поимённо"
            )
            continue
