...
ALLOWED_TEST_ONLY: set[tuple[str, str]] = {
    ...
    # Отображение штриха в фигуру рисунка: незамкнутая ломаная законна для договора, но ни один
    # знак справочника ею не нарисован, и через схему ритуала эта ветвь недостижима. Рабочий код
    # зовёт отображение изнутри модуля, поэтому импортёра снаружи у него нет и не будет.
    ("src/core/presentation/views/diagramView.ts", "figureOf"),
